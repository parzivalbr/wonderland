...
class Connection:
    ...
    def check_packet(self, packet: scapy.packet) -> bool:
        ...
        if packet.sniffed_on == NAT_IFACE and packet.dport == self.NAT_PORT and packet[scapy.IP].src == NAT_IP:
            return self.nat_to_home(packet)
        elif packet.dport == self.HOME_PORT and packet[scapy.IP].src == self.HOME_IP:
    ...
    def icmp_handler(self, packet: scapy.packet) -> bool:
        print("got it lol")
        if packet.sniffed_on == NAT_IFACE and packet[scapy.IP].src == NAT_IFACE:
            if self.SEQ_NUM == packet[scapy.ICMP].seq:
                packet[scapy.IP].dst = self.HOME_IP
                scapy.sendp(packet, iface=HOME_IFACE)
                self.SEQ_NUM += 1
                return True
        elif packet[scapy.IP].src == self.HOME_IP and packet[scapy.ICMP].seq == self.SEQ_NUM:
            print("got it lol")
    ...
    def home_to_nat(self, packet: scapy.packet) -> bool:
        if packet[scapy.IP].src != self.HOME_IP or packet.sport != self.HOME_PORT:
            return False
        packet[scapy.IP].src = NAT_IP
